...
const WidgetsBar = () => {
  ...
  return (
    <div className='widgets-bar  px-0 py-4 flex flex-shrink-0 flex-col gap-4 overflow-y-auto overflow-x-hidden h-[98vh] no-scrollbar  '>
      <WidgetWeather size='S' />
      <WidgetWeather size='M' />
      <WidgetWeather size='L' />
      <WidgetWeather size='L' />
      <div className="action flex justify-center relative align-middle pb-24">
        <AppLauncher appId={12}>
          <button onClick={onWidgetEditClick} className="outline-none bg-black bg-opacity-50 rounded-full border transition-all duration-100 border-gray-500 py-1 text-xs px-4 text-white text-opacity-80 active:text-opacity-90 active:bg-opacity-30">
            Edit Widgets
          </button>
        </AppLauncher>
      </div>
